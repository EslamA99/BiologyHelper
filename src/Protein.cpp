...
class errorsize2 {};///Exception class
class errordefinition2 {};///Exception class
...
Protein::Protein(char * s,Protein_Type atype)///parametrize constructor
{
    ...
    strcpy(seq,s);
    int c=0;
    for(int i=0; i<strlen(s); i++)///if there are forbidden characters in protein
    {
    ...
    if(c!=0)
    {
        Protein:: errordefinition2 ob;///object from class errordefinitions2
        throw ob;///throw the object
    }
    type=atype;
}
...
Protein Protein:: operator +(Protein& otherProtien)
{
    ...
    if (type==otherProtien.type)/// check if the the type of the two is the same
        p.type=type;
    strcpy(p.seq,seq);///add the sequence of the first sequence to  p
    strcat(p.seq,otherProtien.seq);///add the sequence of the second sequence to  p
    return p;
}
bool Protein:: operator ==(Protein otherProtein)///operator ==
{
    if(strcmp(seq,otherProtein.seq)==0&&strlen(seq)==strlen(otherProtein.seq))///check if the sequence of the first protein is equal the sequence of the second object
...
bool Protein:: operator  !=(Protein otherProtein)///operator !=
{
    if(strcmp(seq,otherProtein.seq)!=0)///check if the sequence of the first protein is not equal the sequence of the second object
        return 1;
    else
        return 0;
...
istream&operator >> (istream &in,Protein & obj)
{
    ...
    if(w!=0)
    {
        Protein::errordefinition2 ob;///object from exception class errordefinition2
        throw ob;///throw the object
    }
    ...
    if(c==1)
    {
        obj.type=Hormon;///set value to the type
    }
    if(c==2)
    {
        obj.type=Enzyme;///set value to the type
    }
    if(c==3)
    {
        obj.type=TF;///set value to the type
    }
    if(c==4)
    {
        obj.type=Cellular_Function;///set value to the type
    }
    return in;
...
{
    out<<"Protein sequence= "<<obj.seq<<endl;///print the the sequence of the protein

    out<<"Protein type= "<<P[obj.type-1]<<endl;///print the the type of the protein
    return out;
}
...
void Protein:: add(char*x)
{
    ...
    file<<"\n"<<seq<<" "<<x;///add protein sequence  to file
