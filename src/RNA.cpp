...
RNA::RNA(char * s, RNA_Type atype)///parametrize constructor
{
    ...
    {
        if(s[i]!='A'&&s[i]!='C'&&s[i]!='G'&&s[i]!='U')/// Check for RNA sequence 's 4 characters (A,C,G,U)
            c++;
    }
    if(c!=0)
    {
...
{
    RNA r;
    if (type==otherRNA.type)/// check if the the type of the two RNA is the same
        r.type=type;
    delete[]r.seq;
    ...
    strcat(r.seq,otherRNA.seq);///add the sequence of the second sequence to  p
    return r;///return r
}
